...
exports.prototype.undefine = function(name) {
  this.types[type_case(name)] = undefined;
}
...
function infer (types, obj, path,  parent) {
  ...
  for (var i = 0; i < middleware.length; i++) {
    result = middleware[i].call(types,type, options, explicit)
  ...
  throw new TypeError('Unable to determine schema type of `'+(name || type)+'` at `' + path + '`')
}
...
function filter(object, ignore) {
  var target = {}
  Object.keys(object).forEach(filter_merge(object, target, ignore));
  return target
}
