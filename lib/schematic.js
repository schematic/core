...
function infer (types, obj, path,  parent) {
  ...
  if (info.type instanceof Schema) {
    throw new TypeError(['Type ',
                        ...
                        'Did you mean to pass a constructor?'].join(''));
  }
  var SchemaCtor = getSchemaConstructor(info.type, true);
  if (!SchemaCtor) {
    // Not a Schema constructor, try letting middlware infer the type
    types.emit('infer', info, path, parent);
    SchemaCtor = getSchemaConstructor(info.type, true) || fromName(types, info.type);
  }
  if (SchemaCtor) {
  ...
  if (object) {
    if (type(object) == 'function' && isSchemaConstructor(object))
      return object;
             ...
             && isFunction(object.schema)
             && isSchemaConstructor(object.schema))
